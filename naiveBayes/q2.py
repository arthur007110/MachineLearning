...
intervalo_discretizacao = 256
# Discretização da base de dados original
for label in df_original.columns:
    if label == 'diagnosis':
        continue
    df_original[label] = pd.cut(
        df_original[label], intervalo_discretizacao, labels=False)

# Discretização da base de dados wdbc
for label in df_wdbc.columns:
    if label == 'diagnosis':
        continue
    df_wdbc[label] = pd.cut(
        df_wdbc[label], intervalo_discretizacao, labels=False)

# Discretização da base de dados wpbc
for label in df_wpbc.columns:
    if label == 'outcome':
        continue
    df_wpbc[label] = pd.cut(
        df_wpbc[label], intervalo_discretizacao, labels=False)
...
for train_index, test_index in leave_one_out.split(X):
    ...
    gnb.fit(X_train.values.tolist(), y_train)

    y_pred.append(gnb.predict(X_test.values.tolist())[0])
    y_true.append(y_test.values[0])

accuracy = accuracy_score(y_true, y_pred)

print("Naive Bayes para a base de dados original")
...
print("Naive Bayes para a base de dados wdbc")
print("Acurácia Média:", accuracy)
print("Desvio Padrão:", np.std(y_pred))
print("Intervalo de Confiança:", accuracy - 1.96 * np.std(y_pred) / len(y_true)
      ** 0.5, "a", accuracy + 1.96 * np.std(y_pred) / len(y_true) ** 0.5)
...
print("Naive Bayes para a base de dados wpbc")
print("Acurácia Média:", accuracy)
print("Desvio Padrão:", np.std(y_pred))
print("Intervalo de Confiança:", accuracy - 1.96 * np.std(y_pred) / len(y_true)
      ** 0.5, "a", accuracy + 1.96 * np.std(y_pred) / len(y_true) ** 0.5)
